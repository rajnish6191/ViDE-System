...
<html>
  ...
  <tr><td colspan="4"><i>In Stock</i></td><tr>
    ...
    <table align="center"><tr><td width="100%">
      ...
      if(pageNo>0) {%><a href="./DefaultDisplay.jsp?page=0&title=<%=title%>">First</a>|<a href="./DefaultDisplay.jsp?page=<%=pageNo-1%>&title=<%=title%>"><< |</a>
      <%}
     // int count=0;
      for(int i=0;i<totalpages;i++){
      if((i==pageNo-2)||(i==pageNo-1)||(i==pageNo)||(i==pageNo+1)||(i==pageNo+2)){
      if(!(pageNo==i)){%><a href="DefaultDisplay.jsp?page=<%=i %>&title=<%=title%>"><%}%> <%=(i+1) %></a> | 
      <%//count++;
      }
      //if(count==5)break;
      }
      
      %><%if(pageNo<totalpages-1) {%><a href="DefaultDisplay.jsp?page=<%=pageNo+1%>&title=<%=title%>">>> |</a><a href="DefaultDisplay.jsp?page=<%=totalpages-1%>&title=<%=title%>">Last</a><%} }%></td></tr>
